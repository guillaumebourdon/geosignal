      ...
      if (jinaRes.ok) {
        ...
        for (const href of jinaLinks) {
          ...
          try {
            ...
            const entryLocale = detectLocale(resolved.pathname);
            if (entryLocale !== rootLocale && entryLocale !== 'default') continue;
            seen.add(norm);
...
async function getTopPrioritizedUrls(rootUrl, { maxUrls = 20, timeoutMs = 10000 } = {}) {
    ...
    for (const subXml of results) {
      ...
      if (isSitemapIndex(subXml)) {
        ...
        for (const nestedXml of nestedResults) {
          if (nestedXml && !isSitemapIndex(nestedXml)) allEntries.push(...parseSitemapUrls(nestedXml));
        }
      } else {
  ...
  // If sitemap was found but yielded no usable entries, fall back to crawler
  if (allEntries.length === 0 && xml) {
    console.log(`[sitemapPrioritizer] Sitemap found but empty/unparseable, falling back to crawler`);
    const crawled = await crawlFallback(rootUrl);
    if (crawled.length > 0) allEntries = crawled;
  }
