...
async function fetchWithTimeout(url, timeoutMs) {
  ...
  try {
    ...
    if (url.endsWith('.gz')) {
      ...
      clearTimeout(timer);
      if (!res.ok) {
        console.log(`[sitemapPrioritizer] .gz fetch ${res.status} for ${url}, trying Jina proxy`);
        // Fallback: fetch .gz via Jina proxy (bypasses anti-bot IP blocks)
        try {
          const jinaHeaders = { Accept: 'text/html' };
          if (process.env.JINA_API_KEY) jinaHeaders.Authorization = `Bearer ${process.env.JINA_API_KEY}`;
          const jinaRes = await fetch(`https://r.jina.ai/${url}`, { headers: jinaHeaders, signal: AbortSignal.timeout(15000) });
          if (jinaRes.ok) {
            const jinaText = await jinaRes.text();
            // Jina converts XML sitemaps to markdown with links — extract URLs
            const mdUrls = [...jinaText.matchAll(/\[([^\]]*)\]\((https?:\/\/[^\s)]+)\)/g)].map(m => m[2]);
            if (mdUrls.length > 0) {
              console.log(`[sitemapPrioritizer] .gz via Jina: ${mdUrls.length} URLs from ${url.split('/').pop()}`);
              return `<?xml version="1.0" encoding="UTF-8"?><urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">${mdUrls.map(u => `<url><loc>${u}</loc></url>`).join('')}</urlset>`;
            }
          }
        } catch {}
