...
async function fetchWithTimeout(url, timeoutMs) {
  ...
  try {
    ...
    // Handle .gz (gzip-compressed) sitemaps
    if (url.endsWith('.gz')) {
      const contentEncoding = (res.headers.get('content-encoding') || '').toLowerCase();
      const buf = Buffer.from(await res.arrayBuffer());
      // If server already decompressed (transparent encoding), buf is already XML
      ...
      try {
        ...
        console.log(`[sitemapPrioritizer] gunzip failed for ${url}: ${gzErr.message}`);
        return null;
      }
    }
