...
async function fetchSitemapViaJina(url) {
  try {
    const jinaHeaders = { Accept: 'text/html' };
    if (process.env.JINA_API_KEY) jinaHeaders.Authorization = `Bearer ${process.env.JINA_API_KEY}`;
    const res = await fetch(`https://r.jina.ai/${url}`, { headers: jinaHeaders, signal: AbortSignal.timeout(15000) });
    if (!res.ok) return null;
    const text = await res.text();
    // Jina may return raw XML
    if (text.includes('<urlset') || text.includes('<sitemapindex') || text.includes('<?xml')) return text;
    // Jina converts XML to markdown — extract URLs and build synthetic XML
    const mdUrls = [...text.matchAll(/\[([^\]]*)\]\((https?:\/\/[^\s)]+)\)/g)].map(m => m[2]);
    if (mdUrls.length > 0) {
      console.log(`[sitemapPrioritizer] Jina proxy: ${mdUrls.length} URLs from ${url.split('/').pop()}`);
      // Detect if these are sub-sitemap URLs or page URLs
      const hasSitemapLinks = mdUrls.some(u => u.includes('sitemap'));
      if (hasSitemapLinks) {
        return `<?xml version="1.0" encoding="UTF-8"?><sitemapindex xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">${mdUrls.filter(u => u.includes('sitemap')).map(u => `<sitemap><loc>${u}</loc></sitemap>`).join('')}</sitemapindex>`;
      }
      return `<?xml version="1.0" encoding="UTF-8"?><urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">${mdUrls.map(u => `<url><loc>${u}</loc></url>`).join('')}</urlset>`;
    }
    return null;
  } catch {
    return null;
  ...
  try {
    ...
    if (url.endsWith('.gz')) {
      ...
      if (!res.ok) {
        ...
        // Fallback 2: fetch via Jina proxy (works for text XML, not binary .gz)
        const jinaResult = await fetchSitemapViaJina(nonGzUrl);
    ...
    // Jina fallback for sitemap URLs blocked by anti-bot
    if (url.includes('sitemap')) {
      return await fetchSitemapViaJina(url);
    }
    return null;
  } catch (err) {
    // Jina fallback on exception too
    if (url.includes('sitemap')) {
      try { return await fetchSitemapViaJina(url); } catch { return null; }
    }
    return null;
  }
}
...
async function crawlFallback(rootUrl, { maxDepth = 2, maxUrls = 50, timeoutMs = 15000 } = {}) {
  ...
  // Level 0: fetch homepage (with Jina fallback)
  let homepageHtml = await fetchWithTimeout(rootUrl, 8000);
  if (!homepageHtml) {
    // Jina fallback — extract links from markdown
    try {
      const jinaHeaders = { Accept: 'text/html' };
      if (process.env.JINA_API_KEY) jinaHeaders.Authorization = `Bearer ${process.env.JINA_API_KEY}`;
      const jinaRes = await fetch(`https://r.jina.ai/${rootUrl}`, { headers: jinaHeaders, signal: AbortSignal.timeout(15000) });
      if (jinaRes.ok) {
        const md = await jinaRes.text();
        // Convert Jina markdown links to fake HTML for extractLinks()
        const fakeHtml = md.replace(/\[([^\]]*)\]\((https?:\/\/[^\s)]+)\)/g, '<a href="$2">$1</a>');
        homepageHtml = fakeHtml;
        console.log(`[sitemapPrioritizer] Crawler: homepage fetched via Jina proxy`);
      }
    } catch {}
  }
...
async function getTopPrioritizedUrls(rootUrl, { maxUrls = 20, timeoutMs = 10000 } = {}) {
  ...
  try {
    ...
    if (!homepageHtml) {
      ...
      if (jinaRes.ok) {
        const md = await jinaRes.text();
        homepageHtml = md.replace(/\[([^\]]*)\]\((https?:\/\/[^\s)]+)\)/g, '<a href="$2">$1</a>');
        console.log(`[sitemapPrioritizer] Homepage HTML fetched via Jina for prominence counting`);
      }
    }
  } catch (e) {
