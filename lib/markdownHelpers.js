...
function mdExternalLinks(raw, hostname) {
  ...
  while ((m = regex.exec(raw)) !== null) {
    ...
    try {
      ...
      // Skip localhost, own domain, and already-seen domains
      if (linkHost === 'localhost' || linkHost === '127.0.0.1') continue;
      if (hostname && linkHost.includes(hostname.replace(/^www\./, ''))) continue;
      if (seenDomains.has(linkHost)) continue;
