...
function scoreCitability($, text, raw, locale = 'fr', directHtml = '') {
  ...
  // Answer capsules: 15-70 word blocks immediately after H2s (0-8 pts)
  // Check Markdown (raw) — newlines are preserved
  const mdLines = raw.split(/\n+/).map(l => l.trim()).filter(l => l.length > 0);
  let capsuleCount = 0;
  const headingRegex = /^#{1,3}\s+/;
  for (let i = 0; i < mdLines.length - 1; i++) {
    if (headingRegex.test(mdLines[i])) {
      const next = mdLines[i + 1];
  ...
  capsuleCount = Math.min(capsuleCount, 20); // cap dedup
  ...
  // Use raw content (preserves structure) for analysis
  const rawLen = raw.length;
  const first30Raw = raw.slice(0, Math.floor(rawLen * 0.3));
  const first30Words = first30Raw.split(/\s+/).filter(w => w.length > 1).length;
  const totalWords = raw.split(/\s+/).filter(w => w.length > 1).length;
  // Check: does the first 30% have real content (not just nav/boilerplate)?
  const first30HasHeadings = /^#{1,3}\s+/m.test(first30Raw);
  const first30HasSubstance = first30Words >= 50 && first30HasHeadings;
  if (first30HasSubstance && first30Words >= 100) { score += 5; details.push(`${e.frontLoaded} ✓`); }
