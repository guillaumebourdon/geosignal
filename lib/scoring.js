...
function scoreExtractibility($, text, raw, locale = 'fr', directHtml = '') {
  ...
  // Lists: use direct HTML if available (Jina Markdown parsed by Cheerio won't have <li>)
  let $struct = $;
  if (directHtml) {
    try { $struct = require('cheerio').load(directHtml); } catch {}
  }
  const htmlListCount = $struct('ul li, ol li').filter((_, el) => !$struct(el).closest('nav, header, footer, [role="navigation"], [role="menu"], [role="menubar"], [role="listbox"], [role="tablist"], aside').length).length;
  ...
  const htmlTableCount = $struct('table').filter((_, el) => {
    if ($struct(el).attr('role') === 'presentation') return false;
    if ($struct(el).find('th').length === 0 && $struct(el).find('tr').length <= 1) return false;
    return true;
  }).length;
  ...
  // Headings: pick the most reliable source
  // Direct HTML = real server HTML (misses JS-rendered headings)
  // Jina = headless browser output as Markdown (Cheerio can't parse MD headings)
  // MD regex = parses Jina Markdown headings (# / ## / ###)
  ...
  const mdH1Count = (raw.match(/^# (?!#)[^\n]+/gm) || []).length;
  const mdH2Count = (raw.match(/^## (?!#)[^\n]+/gm) || []).length;
  const mdH3Count = (raw.match(/^### (?!#)[^\n]+/gm) || []).length;
  const mdTotal = mdH1Count + mdH2Count + mdH3Count;
  // If direct HTML found headings, trust it. Otherwise use Markdown (Jina's headless browser).
  const h1Count = directTotal > 0 ? directH1 : mdH1Count;
  const h2Count = directTotal > 0 ? directH2 : mdH2Count;
  const h3Count = directTotal > 0 ? directH3 : mdH3Count;
  ...
  const htmlShortParas = Array.from($struct('p')).filter(p => { const txt = $struct(p).text().trim(); return txt.length > 50 && txt.length < 300; }).length;
...
function scoreVerifiability($, text, html, siteHostname, locale = 'fr') {
  ...
  // Units require word boundaries to avoid false positives (e.g. "2m" as meters, not millions)
  ...
  // k/M (thousands/millions) only when preceded by a number and followed by word boundary or space
  const bigNumbers = text.match(/\d+[.,]?\d*\s*[kK]\b|\d+[.,]?\d*\s*M\b/g) || [];
  // Currencies that need word boundaries to avoid false positives (e.g. "6da" in a hash)
  const currencyNumbers = text.match(/\d+[.,]?\d*\s*(?:DA|DH|TND|FCFA|CFA|£|¥)\b/g) || [];
  const rawNumbers_all = [...rawNumbers, ...bigNumbers, ...currencyNumbers];
  const hourMatches = text.match(/\b\d{1,2}h\d{0,2}\b/gi) || [];
  const allNumbers = [...rawNumbers_all, ...hourMatches];
...
function scoreAuthority($, html, locale = 'fr', directHtml = '') {
  ...
  if (links.some(l => /\/(contact|nous-contacter|contactez-nous)(\/|$|\?|#)/i.test(l) || l.includes('mailto:'))) { score += 3; details.push(`${e.contactPage} ✓`); }
  if (links.some(l => /\/(mentions?[-_]?l[eé]gales?|legal[-_]?notice|imprint)(\/|$|\?|#)/i.test(l) || l.includes('cgu') || l.includes('terms') || l.includes('privacy') || l.includes('confidential'))) { score += 2; details.push(`${e.legalNotices} ✓`); }
  if (links.some(l => /\/(a-propos|about|about-us|qui-sommes-nous|notre-equipe|team)(\/|$|\?|#)/i.test(l))) { score += 2; details.push(`${e.aboutPage} ✓`); }
...
function scoreCrawlability($, html, locale = 'fr', directHtml = '') {
  ...
  if (html.includes('sitemap.xml') || html.includes('sitemap_index') || /href=["'][^"']*sitemap/i.test(html) || (directHtml && (directHtml.includes('sitemap.xml') || directHtml.includes('sitemap_index') || /href=["'][^"']*sitemap/i.test(directHtml)))) { score += 2; details.push(`${e.sitemapFound} ✓`); }
  // Only credit AI bot configuration if mentioned in robots.txt-like context
  const aiBotsConfigured = /user-agent:\s*(gptbot|claudebot|perplexitybot|google-extended|oai-searchbot)/i.test(html);
  if (aiBotsConfigured) {
...
function scoreStructuredData($, html, locale = 'fr') {
  ...
  // Microdata detection
  $('[itemtype]').each((_, el) => {
    const type = ($(el).attr('itemtype') || '').replace(/https?:\/\/schema\.org\//i, '');
  ...
  const medValueTypes = ['Organization', 'Corporation', 'LocalBusiness', 'GovernmentOrganization', 'EducationalOrganization', 'Person', 'Product', 'Service', 'WebSite', 'FinancialProduct', 'Event', 'Course', 'Recipe', 'JobPosting', 'VideoObject', 'Restaurant', 'Dentist', 'Physician', 'Attorney', 'LegalService', 'RealEstateAgent', 'Store', 'AutoRepair', 'MedicalBusiness', 'FinancialService', 'InsuranceAgency', 'TravelAgency', 'FoodEstablishment', 'HealthAndBeautyBusiness', 'SportsActivityLocation', 'Pharmacy', 'Notary', 'RealEstateListing', 'BarberShop', 'BeautySalon', 'HairSalon', 'ChildCare', 'DrivingSchool', 'LodgingBusiness', 'Campground', 'BedAndBreakfast', 'PerformingArtsTheater', 'Museum', 'MovieTheater', 'NursingHome', 'VeterinaryCare', 'Florist', 'AutoDealer', 'Optician', 'BookStore', 'ArtGallery', 'Locksmith', 'Plumber', 'Electrician', 'HVACBusiness', 'RoofingContractor', 'TattooParlor', 'PetStore', 'AnimalShelter', 'EmploymentAgency', 'ProfessionalService', 'GymHealthClub', 'Hostel', 'Hotel', 'Motel'];
...
function scoreExternalPresence($, html, locale = 'fr', directMeta = {}) {
  ...
  // Detect social media iframes (YouTube, Instagram, Facebook, TikTok embeds count as social presence)
  const socialIframes = [];
  ...
  const hasSocial = externalLinks.some(l => (l.includes('linkedin') || l.includes('twitter') || l.includes('x.com') || l.includes('facebook') || l.includes('instagram') || l.includes('youtube') || l.includes('tiktok.com') || l.includes('snapchat.com') || l.includes('pinterest.com') || l.includes('threads.net')) && !socialTrackingExclude.some(e => l.includes(e)))
    ...
    || socialIframes.length > 0;
  if (hasSocial) { score += 2; details.push(`${e.socialMedia} ✓`); }
  if (/t[eé]moignage|avis\s+(client|google|v[eé]rifi[eé]|trustpilot)|customer\s+reviews?|testimonials?|note\s+google|\b[45][.,]\d\s*\/\s*5\b|recommandations?\s+client/i.test(html)) { score += 1; details.push(`${e.testimonials} ✓`); }
...
function scoreFreshness($, html, locale = 'fr') {
  ...
  if (dateModifiedFound) { score += 2; details.push(`${e.dateModified} ✓`); }
