...
function scoreNeutrality($, text, locale = 'fr') {
  ...
  if (wordCount < 50) return { score: 5, max: 10, detail: 'Contenu insuffisant pour evaluer la neutralite' };

  // 1. Superlatives without proof (-0.5 per occurrence, max -3)
  const superlatives = textLower.match(/\b(meilleur|best|n°\s*1|number\s*one|leader|unique|révolutionn|exceptionnel|incomparable|inégalé|unmatched|unrivaled|world.?class|premium|parfait|perfect|ultime|ultimate)\b/gi) || [];
  const supPenalty = Math.min(superlatives.length * 0.5, 3);
  if (supPenalty > 0) {
    score -= supPenalty;
    details.push(`${superlatives.length} superlatif${superlatives.length > 1 ? 's' : ''} detecte${superlatives.length > 1 ? 's' : ''}`);
  }
  ...
  if (unsubPenalty > 0) {
    score -= unsubPenalty;
    details.push(`${unsubstantiated.length} affirmation${unsubstantiated.length > 1 ? 's' : ''} non sourcee${unsubstantiated.length > 1 ? 's' : ''}`);
  }
  ...
  if (ctaDensity > 1.5) {
    score -= 2;
    details.push('Densite elevee d\'appels a l\'action');
  } else if (ctaDensity > 0.8) {
    score -= 1;
    details.push('Appels a l\'action presents');
  }
  ...
  if (educational.length >= 3) {
    ...
    if (details.length === 0) details.push('Ton educatif et factuel');
  }

  score = Math.max(0, Math.min(10, Math.round(score)));

  if (details.length === 0) details.push('Ton equilibre et factuel');
