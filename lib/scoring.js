...
function scoreVerifiability($, text, html, siteHostname, locale = 'fr') {
  ...
  const uniqueNumbers = [...new Set(allNumbers.map(n => n.trim().toLowerCase()))];
...
function scoreExternalPresence($, html, locale = 'fr', directMeta = {}) {
  ...
  if (/t[eé]moignage|avis\s+client|customer\s+reviews?|testimonials?/i.test(html)) { score += 1; details.push(`${e.testimonials} ✓`); }
