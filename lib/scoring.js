...
function scoreExtractibility($, text, raw, locale = 'fr') {
  ...
  const htmlListCount = $('ul li, ol li').filter((_, el) => !$(el).closest('nav, header, footer, [role="navigation"], [role="menu"], [role="menubar"], [role="listbox"], [role="tablist"], aside').length).length;
  ...
  const htmlTableCount = $('table').filter((_, el) => {
...
function scoreVerifiability($, text, html, siteHostname, locale = 'fr') {
  ...
  const rawNumbers = text.match(/\d+[.,]?\d*\s*(%|€|\$|k|M|pts?|points?|fois|times|ans?|years?|mois|months?|jours?|days?|m²|km|kg|DA|DH|TND|FCFA|CFA|£|¥|heures?)/gi) || [];
  const hourMatches = text.match(/\b\d{1,2}h\d{0,2}\b/gi) || [];
  const allNumbers = [...rawNumbers, ...hourMatches];
  ...
  let hasDate = html.includes('datePublished') || html.includes('dateModified') || /\b(20\d{2})\b/.test(text.slice(0, 500)) || $('time[datetime]').length > 0;
  if (!hasDate) {
