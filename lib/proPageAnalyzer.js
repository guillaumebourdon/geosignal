...
const PRO_CACHE_PREFIX = 'detekia:pro:v4:page'; // v4: direct HTML fetch for JSON-LD/meta detection, vocabulary calibration
...
NEVER say "catastrophique" for a score above 30%. Match adjectives to the actual percentage.
...
async function analyzePage(url, { locale = 'fr', timeoutMs = 50000 } = {}) {
  ...
  try {
    ...
    const scores = {
      ...
      authority: scoreAuthority($html, directHtml || rawContent),
      crawlability: scoreCrawlability($html, directHtml || rawContent),
      structuredData: scoreStructuredData($html, directHtml || rawContent),
