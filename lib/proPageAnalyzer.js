...
const PRO_CACHE_PREFIX = 'detekia:pro:v10:page'; // v10: scoring only, no Claude/GPT per page
...
const { scoreCitability, scoreVerifiability, scoreAuthority, scoreAccessibility, scoreExternalPresence, scoreFreshness, scoreNeutrality } = require('./scoring');
...
async function analyzePage(url, { locale = 'fr', timeoutMs = 60000 } = {}) {
  ...
  try {
    ...
    try {
      ...
      if (typeof data === 'string' && data.length > 500) {
        ...
        if (textTest.length > 500) {
          rawContent = data;
          console.log(`[proPageAnalyzer] Direct HTML: ${textTest.length} chars for ${url}`);
        }
      }
    } catch (e) {
      console.log(`[proPageAnalyzer] Direct fetch failed for ${url}: ${e.message.slice(0, 50)}`);
    }

    // Step 2: Browserless for SPAs and thin direct HTML
    if (!rawContent || cheerio.load(rawContent)('body').text().replace(/\s+/g, ' ').trim().split(/\s+/).length < 200) {
      try {
        ...
        if (rendered) {
          ...
          if (brText.length > currentText.length && brText.length > 500) {
            console.log(`[proPageAnalyzer] Browserless: ${brText.length} chars (direct was ${currentText.length}) for ${url}`);
            rawContent = rendered;
            if (!directHtml) directHtml = rendered;
          }
        }
      } catch (e) { console.log(`[proPageAnalyzer] Browserless failed: ${e.message.slice(0, 50)}`); }
    }

    if (!rawContent) return { url, error: 'Scraping failed: unable to fetch content', analyzedAt: new Date().toISOString() };
    ...
    // Use direct HTML for structured data, meta tags, and authority signals
    ...
    const scores = {
      ...
      neutrality:       scoreNeutrality($, textContent, locale_),
    ...
    // Detect embedded content (iframes) from direct HTML
    const proIframes = [];
    ...
    // Pro worker: scoring only — no Claude recos, no citation test per page
    // Claude recos are generated per-page by consolidation's Sonnet dedup pass
    // Citation test is done once for the whole site in consolidation (30 queries)
    // This keeps each worker under 60s instead of 250s, eliminating rate limits
    const rawTotal = Object.values(scores).reduce((s, c) => s + c.score, 0);
    const totalScore = Math.max(0, Math.min(100, rawTotal));

    // Generate simple verdict from scores (no Claude needed)
    ...
    const verdict = totalScore >= 75
      ? `Page avec un bon niveau GEO, force en ${criteriaLabels[strongest[0]]}.`
      : totalScore >= 50
      ? `Page avec des bases correctes, améliorer la ${criteriaLabels[weakest[0]]}.`
      : `Page nécessitant des optimisations, priorité sur la ${criteriaLabels[weakest[0]]}.`;
    const topPriority = `Améliorer la ${criteriaLabels[weakest[0]]} (${weakest[1].score}/${weakest[1].max}).`;
    const strengths = [strongest[1].detail?.substring(0, 100) || `${criteriaLabels[strongest[0]]} performant`];
    ...
    const result = {
      ...
      criteria: [
        ...
        { name: 'Neutralite editoriale', ...scores.neutrality },
        { name: 'Presence externe', ...scores.externalPresence },
        { name: 'Fraicheur & signaux temporels', ...scores.freshness },
      ],
      recommendations: [], // Recos generated at consolidation level, not per-page
      evidence,
      citationTest: null, // Citation test done once in consolidation (30 queries)
      analyzedAt: new Date().toISOString(),
    };
