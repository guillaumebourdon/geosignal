...
const PRO_CACHE_PREFIX = 'detekia:pro:v7:page'; // v7: headings from direct HTML, evidence block, og:title, code disclaimers
...
async function analyzePage(url, { locale = 'fr', timeoutMs = 50000 } = {}) {
  ...
  try {
    ...
    const scores = {
      extractibility: scoreExtractibility($, textContent, rawContent, locale_, directHtml || ''),
