...
async function triggerAutoRefund({ paymentIntentId, customerEmail, url, plan, reason, locale = 'fr' }) {
  ...
  // 1. Stripe refund (with 3 retries, exponential backoff: 2s, 4s, 8s)
  let refundId = null;
  if (paymentIntentId) {
    ...
    for (let attempt = 0; attempt < 3; attempt++) {
      try {
        ...
        refundId = refund.id;
        console.log(`[autoRefund] Refund ${refundId} created for ${paymentIntentId} (attempt ${attempt + 1})`);
        break;
      } catch (e) {
        console.error(`[autoRefund] Stripe refund attempt ${attempt + 1}/3 failed:`, e.message);
        if (attempt < 2) {
          await new Promise(r => setTimeout(r, delays[attempt]));
        }
        // Continue to send alerts even if all retries fail
      }
    }
  }
