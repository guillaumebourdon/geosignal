...
const WHY = {
  ...
  crawlabilite: "La crawlabilite IA mesure si les robots des moteurs IA (GPTBot, ClaudeBot, PerplexityBot) peuvent acceder a votre site et lire son contenu. Si votre robots.txt bloque ces robots ou si votre contenu est rendu uniquement en JavaScript, les IA ne peuvent pas vous citer. Debloquer les crawlers est le quick win le plus rapide, impact visible en 2 a 4 semaines.",
...
function generateProReportHTML(report, locale = 'fr') {
  ...
  const criteriaPages = criteriaOrder.map((criterionName, idx) => {
    ...
    const CRITERION_KEYWORDS = {
      ...
      'Crawlabilite IA': ['crawlab', 'indexab', 'robots', 'seo technique', 'performance technique', 'core web', 'accessibilit', 'performance et ux', 'meta description', 'meta et balises', 'optimisation seo', 'canonique', 'optimisation technique'],
