 ...
 * Same structure as one-page report but with consolidated data from 20 pages.
 * Target: 25-35 pages, NOT 100+ pages.
 */
...
// ── Shared layout constants (same as one-page) ─────────────────────────────
...
// ── Reco detail renderer (same as one-page) ────────────────────────────────
function recoDetail(reco) {
  if (!reco) return '';
  const fields = [
...
// ── Why / Cases / Guides (same content as one-page) ────────────────────────
const WHY = {
...
function generateProReportHTML(report, locale = 'fr') {
  ...
  // ══════════════════════════════════════════════════════════════════════════
  // PAGE 2: EXECUTIVE SUMMARY (same layout as one-page)
  // ══════════════════════════════════════════════════════════════════════════
  ...
  // ══════════════════════════════════════════════════════════════════════════
  // PAGE 4: CONTEXT 2026 (same as one-page)
  // ══════════════════════════════════════════════════════════════════════════
  const ctxCards = [
  ...
  // ══════════════════════════════════════════════════════════════════════════
  // PAGES 5-6: CITATION TEST (30 queries, same design as one-page)
  // ══════════════════════════════════════════════════════════════════════════
  ...
  // ══════════════════════════════════════════════════════════════════════════
  // PAGES 7-14: 8 CRITERIA (site-level, all recos grouped by criterion)
  // ══════════════════════════════════════════════════════════════════════════
  const criteriaOrder = [
  ...
  const criteriaPages = criteriaOrder.map((criterionName, idx) => {
    ...
    for (const r of matchedRecos) {
      const key = (r.title || r.diagnostic || '').slice(0, 50).toLowerCase();
      if (seen.has(key)) {
        const existing = deduped.find(d => (d.title || d.diagnostic || '').slice(0, 50).toLowerCase() === key);
    ...
    const recosHTML = deduped.map((r, ri) => {
      ...
      const pagesNote = r._pages.length > 1 ? `<div style="margin-top:8px;font-family:monospace;font-size:10px;color:#8A8680;">${r._pages.length} pages concernees : ${r._pages.slice(0, 4).map(u => esc(u.replace(rootUrl, '/'))).join(', ')}${r._pages.length > 4 ? ' +' + (r._pages.length - 4) : ''}</div>` : `<div style="margin-top:8px;font-family:monospace;font-size:10px;color:#8A8680;">Page : ${esc(r._pages[0] || '')}</div>`;

      return `<div style="border:1px solid ${rc.color}28;border-left:4px solid ${rc.color};border-radius:0 14px 14px 0;overflow:hidden;margin-bottom:12px;page-break-inside:avoid;">
    ...
    <div style="${P}">
      ...
      ${cc.synthesis ? `<div style="margin-bottom:24px;"><div style="font-family:monospace;font-size:9px;color:#D97757;letter-spacing:2px;text-transform:uppercase;margin-bottom:10px;">Ce que nous avons trouve</div><div style="background:#F7F5F2;border-left:3px solid ${cg.color};padding:14px 18px;border-radius:0 6px 6px 0;font-family:system-ui;font-size:13px;color:#1A1916;line-height:1.65;">${esc(cc.synthesis)}</div></div>` : ''}
      ...
      ${guide ? `<div style="background:rgba(217,119,87,0.04);border-left:3px solid #D97757;border-radius:0 10px 10px 0;padding:18px 22px;margin-bottom:16px;page-break-inside:avoid;"><div style="font-family:monospace;font-size:9px;color:#D97757;letter-spacing:1.5px;text-transform:uppercase;margin-bottom:10px;">Guide technique</div><p style="font-family:system-ui;font-size:13px;color:#3A3835;line-height:1.8;">${esc(guide)}</p></div>` : ''}
      ${caseStudy ? `<div style="background:#E8F7F3;border:1px solid rgba(16,163,127,0.2);border-radius:10px;padding:18px 22px;page-break-inside:avoid;"><div style="font-family:monospace;font-size:9px;color:#10A37F;letter-spacing:1.5px;text-transform:uppercase;margin-bottom:10px;">Cas reel documente</div><p style="font-family:system-ui;font-size:12px;color:#1A1916;line-height:1.7;">${esc(caseStudy)}</p></div>` : ''}
    </div>`;
  }).join('');

  // ══════════════════════════════════════════════════════════════════════════
  // PAGE 15-16: PATTERNS + ACTION PLAN
  // ══════════════════════════════════════════════════════════════════════════
  const patternsCards = (patterns || []).map(p => `<div style="background:#FAFAF9;border:1px solid #E5E2DC;border-radius:10px;padding:16px 20px;margin-bottom:10px;page-break-inside:avoid;">
  ...
  // ══════════════════════════════════════════════════════════════════════════
  // PAGE: RECAP PAR PAGE (compact — 1-2 pages max)
  // ══════════════════════════════════════════════════════════════════════════
  ...
  // ══════════════════════════════════════════════════════════════════════════
  // PAGE: METHODOLOGY (same as one-page)
  // ══════════════════════════════════════════════════════════════════════════
  const methodologyPage = `
  <div style="${P}">
    ${sLabel('Transparence')}
    ${H1('Methodologie')}
    <div style="background:rgba(217,119,87,0.06);border-left:3px solid #D97757;border-radius:0 8px 8px 0;padding:16px 20px;margin-bottom:28px;"><p style="font-family:system-ui;font-size:12px;color:#3A3835;line-height:1.7;">Ce rapport est genere par analyse automatisee de chaque page via un service de scraping specialise et evaluation sur 8 criteres ponderes. Le critere Neutralite editoriale est evalue par intelligence artificielle.</p></div>
    <div style="background:#F7F5F2;border-radius:10px;padding:22px 26px;margin-bottom:32px;"><div style="font-family:monospace;font-size:9px;color:#10A37F;letter-spacing:1.5px;text-transform:uppercase;margin-bottom:10px;">Source academique</div><p style="font-family:system-ui;font-size:12px;color:#1A1916;line-height:1.65;">"Generative Engine Optimization" &mdash; Aggarwal et al., Princeton / Georgia Tech, KDD 2024.</p></div>
    <div style="text-align:center;padding-top:20px;"><div style="font-family:monospace;font-size:10px;color:#B0ABA5;">Rapport genere le ${date} &middot; detekia.fr</div></div>
  </div>`;
