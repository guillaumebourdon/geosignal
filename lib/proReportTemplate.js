...
const { recoSections, esc, S, criterionGrade: sharedCriterionGrade, stripAccents, translateCriterionName } = require('./oneReportTemplate');
...
// Tips for high-scoring but non-perfect criteria (displayed when no recos but score < 100%)
const GOOD_SCORE_TIPS = {
  citabilite: "Votre citabilite est deja solide. Pour atteindre le maximum, verifiez que chaque page commence par une reponse directe a la question principale (answer capsule) et que tous les titres H2/H3 sont formules en questions.",
  extractibilite: "Votre citabilite est deja solide. Pour atteindre le maximum, verifiez que chaque page commence par une reponse directe a la question principale (answer capsule) et que tous les titres H2/H3 sont formules en questions.",
  verifiabilite: "Votre verifiabilite est bonne. Pour aller plus loin, ajoutez 1 a 2 liens externes supplementaires vers des sources de reference sur chaque page cle, et assurez-vous que chaque statistique citee est accompagnee de sa source et de sa date.",
  autorite: "Votre autorite E-E-A-T est bien etablie. Pour la renforcer, ajoutez des biographies d'auteurs detaillees (experience, certifications) et enrichissez votre schema Organization avec les champs foundingDate, founder et sameAs.",
  accessibilite: "Votre accessibilite IA est tres bonne. Pour atteindre 100%, verifiez que votre fichier robots.txt autorise explicitement GPTBot, ClaudeBot et PerplexityBot, et ajoutez un fichier llms.txt a la racine de votre site.",
  crawlabilite: "Votre accessibilite IA est tres bonne. Pour atteindre 100%, verifiez que votre fichier robots.txt autorise explicitement GPTBot, ClaudeBot et PerplexityBot, et ajoutez un fichier llms.txt a la racine de votre site.",
  neutralite: "Votre neutralite editoriale est excellente. Pour la perfectionner, passez en revue les dernieres pages ou des formulations promotionnelles subsistent et remplacez-les par des donnees factuelles sourcees.",
  presence: "Votre presence externe est forte. Pour maximiser ce critere, developpez votre activite sur les plateformes ou vos concurrents sont les plus cites (LinkedIn, Reddit, forums specialises de votre secteur).",
  fraicheur: "Votre fraicheur est correcte. Pour l'optimiser, ajoutez les balises datePublished et dateModified en schema.org sur toutes vos pages, et mettez a jour vos contenus les plus importants avec des donnees recentes.",
};
...
function generateProReportHTML(report, locale = 'fr') {
  ...
  const criteriaRows = Object.entries(normalizedCriteriaAverages).map(([name, data]) => {
    ...
    return `<tr>
      <td style="padding:10px 12px;font-family:system-ui;font-size:12px;color:#1A1916;border-bottom:1px solid #F0EDE8;">${esc(translateCriterionName(name, locale))}</td>
  ...
  <div style="${P}">
    ...
    ${executiveSummary ? `<div style="font-family:system-ui;font-size:13px;color:#3A3835;line-height:1.75;margin-bottom:20px;">${(executiveSummary || '').split('\n').filter(Boolean).map(p => `<p style="margin-bottom:10px;">${esc(p)}</p>`).join('')}</div>` : ''}
    ${(() => {
      const sorted = Object.entries(normalizedCriteriaAverages).map(([name, data]) => ({ name, pct: data.max > 0 ? data.avgScore / data.max : 1 })).sort((a, b) => a.pct - b.pct);
      const weakest3 = sorted.slice(0, 3).map(c => translateCriterionName(c.name, locale).toLowerCase());
      return `<div style="background:#1A1916;border-radius:10px;padding:18px 22px;margin-bottom:28px;"><div style="font-family:monospace;font-size:9px;color:#D97757;letter-spacing:1.5px;margin-bottom:8px;">LECTURE BUSINESS</div><p style="font-family:system-ui;font-size:13px;color:rgba(247,245,242,0.85);line-height:1.65;margin:0;">Le site ne semble pas bloque par l'accessibilite technique. L'enjeu prioritaire est de rendre les pages plus credibles et citables en ameliorant la ${weakest3[0] || 'fraicheur'}, la ${weakest3[1] || 'verifiabilite'} et ${weakest3[2] ? "l'" + weakest3[2] : "l'autorite"}.</p></div>`;
    })()}
  ...
  const queryCards = queries.map(q => {
    ...
    const qType = q.type || q.difficulty || '';
    const typeLabel = qType === 'generic' ? 'GENERIQUE' : qType === 'niche' ? 'NICHE' : 'LONGUE TRAINE';
  ...
  <div style="${P}">
    ...
    <p style="font-family:system-ui;font-size:13px;color:#3A3835;line-height:1.65;margin-bottom:20px;">Les requetes ont ete generees pour simuler differents niveaux d'intention utilisateur : requetes larges, requetes metier, requetes de comparaison et requetes longue traine. Elles visent a verifier si votre site est cite spontanement par les IA sur des recherches liees a votre marche.</p>
    ...
    ${(() => {
      ...
      const longTailQ = queries.filter(q => (q.type || q.difficulty) === 'long_tail');
      const genericCited = genericQ.filter(q => q.cited).length;
      const nicheCited = nicheQ.filter(q => q.cited).length;
      const longTailCited = longTailQ.filter(q => q.cited).length;
      const allCompetitors = queries.flatMap(q => q.competitorsCited || q.competitors_cited || []);
      const competitorFreq = {};
      allCompetitors.forEach(c => { competitorFreq[c] = (competitorFreq[c] || 0) + 1; });
      const topCompetitors = Object.entries(competitorFreq).sort((a, b) => b[1] - a[1]).slice(0, 5).map(([c, n]) => `${esc(c)} (${n}x)`).join(', ');
      const best = genericCited >= nicheCited && genericCited >= longTailCited ? 'Generique' : nicheCited >= longTailCited ? 'Niche' : 'Longue traine';
      const worst = genericCited <= nicheCited && genericCited <= longTailCited ? 'Generique' : nicheCited <= longTailCited ? 'Niche' : 'Longue traine';
      return `<div style="background:#F7F5F2;border-radius:10px;padding:18px 22px;margin-bottom:20px;">
        <div style="font-family:monospace;font-size:9px;color:#D97757;letter-spacing:1.5px;margin-bottom:12px;">RÉSUMÉ PAR CATÉGORIE</div>
        <div style="display:flex;gap:12px;margin-bottom:12px;">
          <div style="flex:1;background:#fff;border-radius:8px;padding:12px 14px;text-align:center;border:1px solid #E5E2DC;"><div style="font-family:monospace;font-size:9px;color:#8A8680;margin-bottom:4px;">GENERIQUE</div><div style="font-family:Georgia,serif;font-size:22px;color:${genericCited > 0 ? '#10A37F' : '#D97757'};">${genericCited}/${genericQ.length}</div></div>
          <div style="flex:1;background:#fff;border-radius:8px;padding:12px 14px;text-align:center;border:1px solid #E5E2DC;"><div style="font-family:monospace;font-size:9px;color:#8A8680;margin-bottom:4px;">NICHE</div><div style="font-family:Georgia,serif;font-size:22px;color:${nicheCited > 0 ? '#10A37F' : '#D97757'};">${nicheCited}/${nicheQ.length}</div></div>
          <div style="flex:1;background:#fff;border-radius:8px;padding:12px 14px;text-align:center;border:1px solid #E5E2DC;"><div style="font-family:monospace;font-size:9px;color:#8A8680;margin-bottom:4px;">LONGUE TRAINE</div><div style="font-family:Georgia,serif;font-size:22px;color:${longTailCited > 0 ? '#10A37F' : '#D97757'};">${longTailCited}/${longTailQ.length}</div></div>
        </div>
        <div style="font-family:system-ui;font-size:11px;color:#3A3835;line-height:1.6;">
          ${topCompetitors ? `<div style="margin-bottom:4px;"><strong>Principaux domaines cites a votre place :</strong> ${topCompetitors}</div>` : ''}
        </div>
      </div>
      <div style="background:rgba(66,133,244,0.06);border-left:3px solid #4285F4;border-radius:0 8px 8px 0;padding:12px 16px;margin-bottom:16px;"><p style="font-family:system-ui;font-size:11px;color:#3A3835;line-height:1.6;margin:0;">Les domaines cites a votre place refletent les reponses reellement generees par les IA. Certains peuvent etre des concurrents directs, des alternatives larges, des acteurs internationaux ou des sources informationnelles selon la requete.</p></div>`;
    })()}
    ${queryCards}
    ...
    <div style="${idx === 0 ? P : 'margin-top:40px;background:#fff;'}">
      <div class="criterion-header" style="margin-bottom:28px;">
        ...
        <div style="display:flex;align-items:flex-end;justify-content:space-between;margin-bottom:10px;gap:16px;">
          <h2 style="font-family:Georgia,serif;font-size:26px;color:#1A1916;letter-spacing:-0.5px;line-height:1.2;margin:0;">${esc(translateCriterionName(criterionName, locale))}</h2>
      ...
      <div style="margin-bottom:24px;">
        ...
        ${recosHTML || (pct >= 100
          ? '<p style="font-family:system-ui;font-size:13px;color:#10A37F;">Tres bon niveau detecte sur les signaux mesures par l\'audit. Aucune action corrective necessaire sur ce critere.</p>'
          : `<div style="background:rgba(16,163,127,0.06);border:1px solid rgba(16,163,127,0.2);border-radius:10px;padding:16px 20px;">
              <div style="display:flex;align-items:flex-start;gap:10px;margin-bottom:8px;">
                <span style="color:#10A37F;font-size:14px;flex-shrink:0;">&#10003;</span>
                <span style="font-family:system-ui;font-size:13px;color:#10A37F;font-weight:600;">Bon niveau (${pct}%)</span>
              </div>
              <p style="font-family:system-ui;font-size:13px;color:#3A3835;line-height:1.65;margin:0;">${esc(lookupMap(GOOD_SCORE_TIPS, criterionName) || 'Ce critere est bien optimise. Quelques ajustements mineurs pourraient encore ameliorer votre score.')}</p>
            </div>`)}
      </div>
  ...
  const projected = (() => {
    ...
    return Math.min(100, scoreAverage + Math.round(gain * 0.5));
  })();
  ...
  <div style="${P}">
    ...
    <div style="margin-top:32px;background:#1A1916;border-radius:14px;padding:24px 28px;margin-bottom:16px;">
      <div style="display:flex;align-items:center;gap:20px;">
        ...
        <div style="flex:1;"><div style="font-family:monospace;font-size:9px;color:#D97757;letter-spacing:1.5px;margin-bottom:6px;">SCORE PROJETE APRES OPTIMISATION</div><div style="font-family:system-ui;font-size:12px;color:rgba(247,245,242,0.7);line-height:1.6;">En appliquant les recommandations de ce rapport, votre score GEO pourrait passer de ${scoreAverage} a ${projected}/100. Le gain projete vient principalement de l'amelioration de ${(() => {
          const sorted = Object.entries(normalizedCriteriaAverages).map(([name, data]) => ({ name, pct: data.max > 0 ? data.avgScore / data.max : 1 })).filter(c => c.pct < 0.75).sort((a, b) => a.pct - b.pct);
          return sorted.slice(0, 3).map(c => translateCriterionName(c.name, locale).toLowerCase()).join(', ') || 'plusieurs criteres';
        })()}.</div></div>
      </div>
    </div>
