...
function generateProReportHTML(report, locale = 'fr') {
  ...
  const criteriaPages = criteriaOrder.map((criterionName, idx) => {
    ...
    // Deduplicate by fuzzy title matching (60%+ word overlap = same reco)
    ...
    for (const r of matchedRecos) {
      const key = dedupKey(r.title || r.problem?.substring(0, 80) || r.diagnostic || '');
      ...
      if (matchedIdx === undefined) {
        ...
        for (const [ek, idx] of Object.entries(keyMap)) {
          const ew = new Set(ek.split(' ').filter(w => w.length > 3));
          const overlap = [...keyWords].filter(w => ew.has(w)).length;
          const minSz = Math.min(keyWords.size, ew.size);
          if (minSz > 0 && overlap / minSz >= 0.6) { matchedIdx = idx; break; }
        }
      }
      if (matchedIdx !== undefined) {
