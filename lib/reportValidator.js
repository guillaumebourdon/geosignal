...
function validateReport(report, reportType = 'onepage') {
  ...
  if (isPro) {
    ...
    const actualValid = (report.pages || []).filter(p => !p.error).length;
    if (pagesValid !== actualValid) {
      errors.push({
  ...
  if (isPro) {
    ...
    if (validPages.length > 0) {
      const calcAvg = Math.round(validPages.reduce((s, p) => s + p.score, 0) / validPages.length);
      if (Math.abs(calcAvg - report.scoreAverage) > 1) {
        errors.push({
          rule: 'score-average-mismatch',
          severity: 'error',
          location: 'scoreAverage',
          expected: String(calcAvg),
          actual: String(report.scoreAverage),
        });
      }
    }
  }
