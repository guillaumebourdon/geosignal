...
function generateReportHTML(data, locale = 'fr') {
  ...
  const F  = 'padding:52px 56px 64px;background:#fff;box-sizing:border-box;'; // Flowing section (no forced break)
  ...
  const criteriaPages = criteria.map((c, idx) => {
   try {
    ...
    return `<div style="${F}"><p style="color:#D97757;font-family:system-ui;">Error rendering criterion: ${esc(c?.name || 'unknown')}</p></div>`;
   }
  }).join('');
  ...
  <div class="beeleven-cta-block" style="${F}display:flex;align-items:center;justify-content:center;padding-top:80px;padding-bottom:80px;">
    <div style="max-width:480px;text-align:center;">
      ${sLabel(t.beeleven.label)}
      ${H1(t.beeleven.h1)}
      <p style="font-family:system-ui;font-size:14px;color:#6B6762;line-height:1.5;margin-bottom:32px;">${t.beeleven.text}</p>
