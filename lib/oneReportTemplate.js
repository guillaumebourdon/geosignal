...
function recoSections(reco, t) {
  ...
  if (reco.problem) {
    ...
    const problemBlock = reco.problem ? `<div style="margin-bottom:16px;">
      <div style="font-family:system-ui;font-size:11px;font-weight:700;color:#D97757;text-transform:uppercase;letter-spacing:1px;margin-bottom:8px;">Le probl\u00e8me</div>
      <div style="font-family:system-ui;font-size:13px;color:#3A3835;line-height:1.7;">${esc(reco.problem)}</div>
    </div>` : '';

    const solutionBlock = reco.solution ? `<div style="margin-bottom:16px;">
      <div style="font-family:system-ui;font-size:11px;font-weight:700;color:#10A37F;text-transform:uppercase;letter-spacing:1px;margin-bottom:8px;">La solution</div>
      <div style="font-family:system-ui;font-size:13px;color:#3A3835;line-height:1.7;">${esc(reco.solution)}</div>
    </div>` : '';
    ...
    const techBlock = reco.technicalImplementation ? (() => {
      ...
      const listItems = steps.map(s => `<li style="margin-bottom:6px;padding-left:4px;">${esc(String(s).replace(/^\d+\.\s*/, ''))}</li>`).join('');
      return `<div style="background:#F7F5F2;border-radius:8px;padding:14px 18px;margin-bottom:16px;">
        <div style="font-family:system-ui;font-size:11px;font-weight:700;color:#1A1916;margin-bottom:8px;">\u{1F6E0}\uFE0F Mise en \u0153uvre technique</div>
        <ol style="font-family:system-ui;font-size:13px;color:#3A3835;line-height:1.7;margin:0;padding-left:20px;">${listItems}</ol>
      </div>`;
    })() : '';

    const codeBlock = reco.codeExample ? `<div style="background:#1A1916;color:#F7F5F2;border-radius:8px;padding:16px;font-family:monospace;font-size:10px;line-height:1.6;white-space:pre-wrap;overflow-wrap:anywhere;word-break:break-word;max-width:100%;box-sizing:border-box;page-break-inside:avoid;break-inside:avoid;">${esc(reco.codeExample)}</div>` : '';
