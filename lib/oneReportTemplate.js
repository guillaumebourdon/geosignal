...
function recoSections(reco, t) {
  ...
  if (reco.problem) {
    ...
    const techBlock = reco.technicalImplementation ? `<div style="background:#F7F5F2;border-radius:8px;padding:14px 18px;margin-bottom:16px;">
      ...
      <div style="font-family:system-ui;font-size:13px;color:#3A3835;line-height:1.7;white-space:pre-wrap;">${esc(reco.technicalImplementation)}</div>
    </div>` : '';

    const codeBlock = reco.codeExample ? `<div style="background:#1A1916;color:#F7F5F2;border-radius:8px;padding:16px;font-family:monospace;font-size:11px;line-height:1.6;white-space:pre-wrap;word-break:break-all;">
      ${esc(reco.codeExample)}
    </div>` : '';
