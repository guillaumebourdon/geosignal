...
function recoSections(reco, t) {
  ...
  if (reco.problem) {
    ...
    const techBlock = reco.technicalImplementation ? (() => {
      ...
      return `<div style="background:#F7F5F2;border-radius:8px;padding:12px 16px;margin-bottom:10px;">
    ...
    const codeBlock = reco.codeExample ? `<div style="margin-bottom:10px;">
      <div style="font-family:system-ui;font-size:11px;font-weight:700;color:#1A1916;margin-bottom:5px;">&lt;/&gt; Exemple de code</div>
      <div style="background:#1A1916;color:#F7F5F2;border-radius:8px;padding:14px;font-family:monospace;font-size:10px;line-height:1.55;white-space:pre-wrap;overflow-wrap:anywhere;word-break:break-word;max-width:100%;box-sizing:border-box;page-break-inside:avoid;break-inside:avoid;">${esc(reco.codeExample)}</div>
    </div>` : '';
...
function generateReportHTML(data, locale = 'fr') {
  ...
  <div style="${P}">
    ...
    <div style="background:#1A1916;border-radius:12px;padding:24px 28px;margin-bottom:36px;display:flex;align-items:center;gap:24px;">
      ...
      <div><div style="font-family:system-ui;font-size:13px;color:rgba(247,245,242,0.65);line-height:1.5;max-width:460px;">${esc(verdict)}</div><div style="font-family:system-ui;font-size:11px;color:rgba(247,245,242,0.28);margin-top:8px;">${t.synthesis.avgNote}</div></div>
    </div>
  ...
  <div style="${P}">
    ...
    <div style="background:#F7F5F2;border:1px solid #E5E2DC;border-radius:14px;padding:28px 32px;margin-bottom:32px;text-align:center;">
      <div style="font-family:Georgia,serif;font-size:18px;color:#1A1916;margin-bottom:8px;letter-spacing:-0.3px;">${t.beeleven.h1}</div>
      <p style="font-family:system-ui;font-size:13px;color:#8A8680;line-height:1.5;max-width:440px;margin:0 auto 16px;">${t.beeleven.text}</p>
      <a href="${beelevenUrl}" style="display:inline-block;background:#D97757;color:#fff;padding:11px 28px;border-radius:9px;font-family:system-ui;font-size:14px;font-weight:700;text-decoration:none;">${t.beeleven.cta}</a>
      <div style="font-family:monospace;font-size:10px;color:#B0ABA5;margin-top:10px;">beeleven.fr &middot; hello@detekia.fr</div>
    </div>
    <div class="score-projection-card" style="background:#1A1916;border-radius:14px;padding:30px 36px;display:flex;align-items:center;gap:32px;">
      ...
      <div style="flex:1;padding-left:28px;border-left:1px solid rgba(247,245,242,0.1);"><div style="font-family:system-ui;font-size:13px;color:rgba(247,245,242,0.55);line-height:1.5;">${t.actionPlan.projectionText} <strong style="color:#F7F5F2;">${score}/100</strong> ${t.actionPlan.projectionTo} <strong style="color:#10A37F;">${projected}/100</strong>.</div></div>
    </div>
    <div style="margin-top:24px;font-family:system-ui;font-size:11px;color:rgba(247,245,242,0.35);line-height:1.6;font-style:italic;">Cette projection est indicative. Elle suppose une implementation propre des recommandations et ne prend pas en compte l'evolution rapide des moteurs IA (nouveaux criteres d'evaluation, changements d'algorithmes de citation, emergence de nouvelles plateformes). Le GEO est un terrain mouvant — l'objectif est de maintenir une veille active et d'ajuster regulierement la strategie de visibilite IA.</div>
  </div>`;
  ...
  <div style="${P}">
    ...
    <div style="background:rgba(217,119,87,0.06);border-left:3px solid #D97757;border-radius:0 8px 8px 0;padding:16px 20px;margin-bottom:28px;">
      <p style="font-family:system-ui;font-size:12px;color:#3A3835;line-height:1.5;">${t.methodology.limitsWarning}</p>
    </div>
    <div style="background:#F7F5F2;border-radius:10px;padding:22px 26px;margin-bottom:32px;">
      <p style="font-family:system-ui;font-size:13px;color:#1A1916;line-height:1.5;margin-bottom:10px;">${t.methodology.howItWorks1}</p>
      <p style="font-family:system-ui;font-size:13px;color:#1A1916;line-height:1.5;margin-bottom:10px;">${t.methodology.howItWorks2}</p>
      <p style="font-family:system-ui;font-size:13px;color:#1A1916;line-height:1.5;">${t.methodology.howItWorks3}</p>
    </div>
...
<style>
  ...
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  /* Reco cards: avoid breaking mid-reco */
  div[style*="border-left: 4px solid"], div[style*="border-left:4px solid"] {
    page-break-inside: avoid; break-inside: avoid;
  }
  /* Code blocks: never split */
  div[style*="background:#1A1916"][style*="monospace"] {
    page-break-inside: avoid; break-inside: avoid;
  }
  /* Tech implementation blocks */
  div[style*="background:#F7F5F2"][style*="border-radius:8px"] {
    page-break-inside: avoid; break-inside: avoid;
  }
