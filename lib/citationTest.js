      ...
      if (!/^(aucune|none|n\/a|-)$/i.test(brandsList)) {
        competitors = brandsList.split(/[,;]/)
          ...
          .filter(b => {
            ...
            if (brandLower.length > 3 && lower.includes(brandLower)) return false;
            return true;
          });
      }
    }
...
function extractCompetitors(response, hostname, brand) {
  ...
  const rejectWords = new Set([
    ...
    // FR common nouns / adjectives / adverbs capitalized at start of sentence
    ...
    // FR common words that get capitalized at sentence start and mistaken for brands
    ...
    'grâce', 'grace', 'outre', 'alors', 'reste', 'reste', 'suite', 'point',
    'partie', 'niveau', 'place', 'forme', 'monde', 'genre', 'type', 'aide',
    'guide', 'notez', 'sachez', 'voyez', 'lisez', 'allez', 'venez', 'faites',
    'dites', 'mieux', 'prise', 'mise', 'bien', 'tout', 'rien', 'plus',
    'selon', 'environ', 'autour', 'travers', 'dessus', 'dessous', 'dedans',
    'dehors', 'durant', 'pendant', 'tandis', 'quant', 'lorsque',
    // Common sentence starters in AI responses
    'cependant', 'toutefois', 'néanmoins', 'neanmoins', 'premièrement',
    'deuxièmement', 'troisièmement', 'finalement', 'egalement', 'également',
    'autrement', 'concernant', 'contrairement', 'comparativement',
    'simplement', 'uniquement', 'seulement', 'absolument', 'certainement',
    // EN common words that get capitalized
    'here', 'these', 'those', 'their', 'about', 'after', 'before', 'since',
    'while', 'where', 'which', 'there', 'being', 'other', 'using', 'based',
    'first', 'second', 'third', 'final', 'overall', 'instead', 'rather',
    // EN common words
  ...
  singleBrands.forEach(m => {
    ...
    // Reject short single words (< 5 chars) — almost never real brands
    // Real brands are typically 5+ chars: Qonto, Swile, Shine, Lydia, Payfit
