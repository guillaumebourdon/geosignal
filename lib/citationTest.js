...
function extractCompetitors(response, hostname, brand) {
  ...
  // e.g., "Club Med", "Center Parcs", "Google Analytics", "Les Villages Clubs"
  const brandMatches = response.match(/\b[A-Z][a-zA-Z]{1,}(?:\s+[A-Z][a-zA-Z]{1,}){1,4}\b/g) || [];

  // Build brand variants for matching (e.g., "Le Coq Sportif" → also match "Coq Sportif")
  ...
  // Filter: only keep multi-word matches that look like real brand names
  const genericMultiWord = new Set([
    // FR generic phrases that get capitalized
    'les alpes', 'les pyrenees', 'les pyrénées', 'la france', 'la corse', 'la bretagne',
    'le monde', 'les etats', 'les états', 'la méditerranée', 'la mediterranee',
    'club de', 'centre de', 'office de', 'chambre de', 'conseil de',
    'pour les', 'dans les', 'avec les', 'sur les', 'par les',
    // EN generic phrases
    'the best', 'the most', 'the first', 'the top', 'how to',
    'for the', 'with the', 'from the', 'this is',
  ]);

  brandMatches.forEach(m => {
    const lower = m.toLowerCase();
    // Skip brand self-references
    if (brandVariants.has(lower) || lower.includes(hostLower)) return;
    if (brandWords.some(bw => bw.length > 3 && lower.includes(bw))) return;
    // Skip generic phrases
    if (genericMultiWord.has(lower)) return;
    // Skip if starts with common article/preposition (likely a phrase, not a brand)
    if (/^(le |la |les |l'|un |une |des |du |de |en |au |aux |the |a |an |for |with |from |this |that |par |sur |dans |pour )/i.test(m)) return;
    // Must be at least 2 words and total length > 4
