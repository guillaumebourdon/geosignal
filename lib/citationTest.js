...
function extractCompetitors(response, hostname, brand) {
  ...
  // Extract brand-like capitalized words — must look like a brand name, not a common word
  // Only match 2+ consecutive capitalized words (e.g. "Google Analytics", "Back Market")
  // or single words that contain unusual patterns for common words (mixed case, numbers, dots)
  const brandMatches = response.match(/\b[A-Z][a-zA-Z]{2,}(?:\s+[A-Z][a-zA-Z]{2,})+\b/g) || [];
  // Also match known tech brand patterns (single capitalized word with 4+ chars that isn't a common word)
  const singleBrands = response.match(/\b[A-Z][a-z]{3,}\b/g) || [];
  const commonWords = new Set([
    // English
    'the', 'this', 'that', 'with', 'from', 'your', 'here', 'these', 'their', 'which', 'when',
    'what', 'some', 'more', 'also', 'best', 'most', 'they', 'will', 'have', 'been', 'each',
    'than', 'only', 'very', 'many', 'both', 'such', 'just', 'like', 'make', 'over', 'well',
    'into', 'about', 'after', 'could', 'would', 'should', 'there', 'where', 'being', 'those',
    'other', 'every', 'under', 'still', 'while', 'using', 'first', 'based', 'since', 'offer',
    // French
    'pour', 'dans', 'avec', 'plus', 'vous', 'nous', 'voici', 'cela', 'cette', 'tout', 'tous',
    'leur', 'mais', 'donc', 'alors', 'comme', 'aussi', 'bien', 'quel', 'quels', 'elle', 'elles',
    'sont', 'sera', 'fait', 'faire', 'avoir', 'etre', 'peut', 'entre', 'sans', 'autre', 'tres',
    'encore', 'avant', 'depuis', 'votre', 'notre', 'ainsi', 'toute', 'toutes', 'contre',
    'mesurer', 'utiliser', 'analyser', 'suivre', 'installer', 'configurer', 'choisir',
    'optimisation', 'protection', 'conformit', 'personnalisation', 'transparence',
    'assurez', 'expliquer', 'comprendre', 'permettre', 'assurer',
    'outils', 'sources', 'donn', 'collecte', 'collect', 'visit', 'visiteurs', 'nombre',
    'trafic', 'suivi', 'analyse', 'contenu', 'framework', 'biblioth',
    'avantages', 'communaut', 'solutions', 'installation',
    // Common FR words that get capitalized at start of sentence
    'france', 'recherche', 'certaines', 'certains', 'sportif', 'sportive',
    'plusieurs', 'notamment', 'exemple', 'cependant', 'toutefois', 'ensuite',
    'enfin', 'souvent', 'parfois', 'seulement', 'effectivement', 'actuellement',
    'concernant', 'globalement', 'principalement', 'alternative', 'alternatives',
    'marque', 'marques', 'produit', 'produits', 'qualit', 'gamme',
    'particulier', 'national', 'international', 'traditionnel', 'moderne',
    // Common EN words that get capitalized
    'however', 'several', 'certain', 'another', 'often', 'always', 'general',
    'specific', 'international', 'national', 'traditional', 'modern', 'original',
    'alternative', 'particular', 'professional', 'available', 'popular', 'important',
  ]);

  // Build brand variants for matching (e.g., "Le Coq Sportif" → also match "Coq Sportif", "Coq")
  const brandWords = brandLower.split(/\s+/).filter(w => w.length > 2);
  const brandVariants = new Set([brandLower]);
  // Add sub-phrases: "le coq sportif" → "coq sportif", "sportif"
  for (let i = 0; i < brandWords.length; i++) {
    brandVariants.add(brandWords.slice(i).join(' '));
  }

  [...brandMatches, ...singleBrands].forEach(m => {
    const lower = m.toLowerCase();
    // Skip if it matches any variant of the brand name
    if (brandVariants.has(lower) || lower.includes(hostLower)) return;
    // Skip if any brand word appears in this match (e.g., "Sportif" when brand is "Le Coq Sportif")
    if (brandWords.some(bw => bw.length > 3 && lower.includes(bw))) return;
    if (lower.length > 3 && !commonWords.has(lower) && !commonWords.has(lower.slice(0, -1))) {
      competitors.add(m);
    }
  });
