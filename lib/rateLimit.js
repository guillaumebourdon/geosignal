...
const limiters = {
  ...
  analyzeDaily: new Ratelimit({ redis, limiter: Ratelimit.fixedWindow(3, '86400 s'), prefix: 'rl:analyze-daily' }),
  ...
  suggestPages: new Ratelimit({ redis, limiter: Ratelimit.slidingWindow(10, '60 s'), prefix: 'rl:suggest-pages' }),
  verifyPayment: new Ratelimit({ redis, limiter: Ratelimit.slidingWindow(10, '60 s'), prefix: 'rl:verify-payment' }),
  proWaitlist: new Ratelimit({ redis, limiter: Ratelimit.slidingWindow(5, '600 s'), prefix: 'rl:pro-waitlist' }),
...
async function checkDailyLimit(req) {
  const limiter = limiters.analyzeDaily;
  if (!limiter) return true;
  const ip = req.headers['x-forwarded-for']?.split(',')[0]?.trim() || req.socket?.remoteAddress || 'unknown';
  try {
...
module.exports = { checkRateLimit, checkDailyLimit };
