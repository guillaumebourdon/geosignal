    ...
    <>
      ...
      <main>
        ...
        <section style={{ maxWidth: 680, margin: '0 auto', padding: '72px 24px 48px', textAlign: 'center' }}>
          <div className="reveal" style={{ display: 'inline-block', fontFamily: 'monospace', fontSize: 10, color: '#6B6762', letterSpacing: 2, textTransform: 'uppercase', marginBottom: 20 }}>
            {t('about.hero.badge')}
          </div>
          ...
          <p className="reveal reveal-d2" style={{ fontSize: 16, color: '#6B6762', lineHeight: 1.65, fontFamily: 'system-ui', maxWidth: 520, margin: '0 auto' }}>
            ...
            <div className="about-principles-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 14 }}>
              {t('about.principles.items').map((item, i) => (
                <div key={i} className="card-interactive" style={{
                  ...
                  <div style={{ fontFamily: 'system-ui', fontSize: 13, color: '#6B6762', lineHeight: 1.6 }}>
                    {item.desc}
                  </div>
                </div>
              ))}
        ...
        <section style={{ maxWidth: 640, margin: '0 auto', padding: '48px 24px' }}>
          <div style={{ fontFamily: 'monospace', fontSize: 10, color: '#6B6762', letterSpacing: 2, textTransform: 'uppercase', marginBottom: 20 }}>
            {t('about.team.label')}
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
            ...
            <div style={{ flex: 1 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                ...
                <span style={{ fontFamily: 'system-ui', fontSize: 12, color: '#6B6762' }}>·</span>
                <span style={{ fontFamily: 'system-ui', fontSize: 13, color: '#6B6762' }}>{t('about.team.role')}</span>
              </div>
              <div style={{ fontFamily: 'system-ui', fontSize: 13, color: '#6B6762', marginTop: 3 }}>{t('about.team.bio')}</div>
            </div>
            <a href={t('about.team.linkedin')} target="_blank" rel="noopener noreferrer" style={{ flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', width: 36, height: 36, borderRadius: 8, border: '1px solid #E5E2DC', color: '#6B6762', textDecoration: 'none', transition: 'border-color 0.2s, color 0.2s' }}
              onMouseEnter={e => { e.currentTarget.style.borderColor = '#0A66C2'; e.currentTarget.style.color = '#0A66C2'; }}
              onMouseLeave={e => { e.currentTarget.style.borderColor = '#E5E2DC'; e.currentTarget.style.color = '#6B6762'; }}>
