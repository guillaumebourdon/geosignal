...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!cached) {
      return res.status(404).send(`<h1 style="font-family:system-ui;padding:40px;">Aucun scan trouvé pour <code>${url}</code>.<br><br>Scannez d'abord le site sur <a href="https://detekia.fr">detekia.fr</a></h1>`);
    }

    const html = generateReportHTML({ url, ...cached });
    res.setHeader('Content-Type', 'text/html; charset=utf-8');
    return res.status(200).send(html);
  } catch (e) {
    console.error('preview-report error:', e);
    return res.status(500).send(`<h1 style="font-family:system-ui;padding:40px;">Erreur : ${e.message}</h1>`);
  }
}
