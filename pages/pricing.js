    ...
    <div style={{ background: '#F7F5F2', minHeight: '100vh', fontFamily: 'Georgia, serif' }}>
      ...
      <div style={{ maxWidth: 860, margin: '0 auto', padding: '0 24px', display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 16, marginBottom: 80 }}>
        ...
        <div style={{ background: '#fff', border: '1px solid #E5E2DC', borderRadius: 16, padding: 28 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            {[
              '✓ Score GEO sur 120',
              '✓ Analyse des 6 critères',
              '✓ 2 recommandations offertes',
              '✗ Recommandations complètes',
              '✗ Plan d\'action détaillé',
              '✗ Analyses illimitées',
            ].map((f, i) => (
              <div key={i} style={{ fontSize: 13, color: f.startsWith('✗') ? '#C2BDB8' : '#3A3835', fontFamily: 'system-ui, sans-serif', display: 'flex', gap: 8 }}>
                {f}
              </div>
            ))}
          </div>
        ...
        {/* RAPPORT */}
        <div style={{ background: '#1A1916', border: '1px solid #1A1916', borderRadius: 16, padding: 28, position: 'relative', transform: 'scale(1.03)', boxShadow: '0 12px 40px rgba(26,25,22,0.15)' }}>
          ...
          <a href="#" style={{ display: 'block', textAlign: 'center', background: '#D97757', color: '#fff', padding: '12px 0', borderRadius: 10, fontWeight: 600, fontSize: 14, textDecoration: 'none', fontFamily: 'system-ui, sans-serif', marginBottom: 28 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            {[
              '✓ Score GEO sur 120',
              '✓ Analyse des 6 critères',
              '✓ 5 recommandations complètes',
              '✓ Plan d\'action priorisé',
              '✓ Rapport PDF téléchargeable',
              '✗ Analyses illimitées',
            ].map((f, i) => (
              <div key={i} style={{ fontSize: 13, color: f.startsWith('✗') ? 'rgba(247,245,242,0.25)' : 'rgba(247,245,242,0.85)', fontFamily: 'system-ui, sans-serif' }}>
                {f}
              </div>
            ))}
          </div>
        ...
        <div style={{ background: '#fff', border: '2px solid #1A1916', borderRadius: 16, padding: 28 }}>
          ...
          <a href="#" style={{ display: 'block', textAlign: 'center', background: '#1A1916', color: '#F7F5F2', padding: '12px 0', borderRadius: 10, fontWeight: 600, fontSize: 14, textDecoration: 'none', fontFamily: 'system-ui, sans-serif', marginBottom: 28 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            {[
              ...
              <div key={i} style={{ fontSize: 13, color: '#3A3835', fontFamily: 'system-ui, sans-serif' }}>
                {f}
              </div>
            ))}
          </div>
      ...
      <div style={{ maxWidth: 620, margin: '0 auto', padding: '0 24px 100px' }}>
        ...
        {[
          ...
          ['Qu\'est-ce que le GEO ?', 'Le GEO (Generative Engine Optimization) est l\'optimisation de votre site pour apparaître dans les réponses de ChatGPT, Claude, Gemini et Perplexity — comme le SEO mais pour les IA.'],
          ['Le paiement à 9€ est-il par site ou par analyse ?', 'Par analyse. Chaque rapport débloqué correspond à une analyse d\'un site à un instant donné. Si vous analysez à nouveau le même site après modifications, c\'est une nouvelle analyse.'],
          ['Puis-je annuler mon abonnement Pro ?', 'Oui, à tout moment et sans engagement. Votre accès reste actif jusqu\'à la fin de la période payée.'],
          ['Les données de mon site sont-elles conservées ?', 'Non. GeoSignal analyse votre site en temps réel et ne conserve aucune donnée de votre site sur nos serveurs.'],
        ].map(([q, a], i) => (
