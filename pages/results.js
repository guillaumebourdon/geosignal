...
const CRITERIA_INFO = {
  'Extractibilité & réponse directe': {
    ...
    desc: 'Les IA cherchent des réponses "prêtes à citer" dans votre contenu. Si votre texte répond clairement dès les premières lignes, avec des listes et des tableaux, les IA peuvent l\'extraire et le réutiliser facilement dans leurs réponses.',
  ...
  'Vérifiabilité & preuves': {
    ...
    desc: 'Les IA citent en priorité les contenus qui prouvent ce qu\'ils avancent : chiffres sourcés, dates précises, liens vers des sources primaires. Un contenu sans preuve est moins fiable et donc moins cité.',
    impact: '20 points',
    color: '#10A37F',
  },
  'Autorité & E-E-A-T': {
    ...
    desc: 'E-E-A-T signifie Expérience, Expertise, Autorité, Confiance. Les IA favorisent les sites dont l\'auteur est identifié, avec une biographie, des credentials visibles et des pages About/Contact claires.',
    impact: '15 points',
    color: '#10A37F',
  },
  'Crawlabilité IA': {
  ...
  'Données structurées': {
    ...
    desc: 'Le Schema.org est un langage que les IA lisent en priorité. Il leur dit explicitement : "ce site est une organisation", "cette page est une FAQ", "cet article a été écrit par untel". Sans lui, les IA doivent deviner.',
    impact: '10 points',
    color: '#4285F4',
  },
  'Neutralité éditoriale': {
    ...
    desc: 'Les IA évitent de citer des contenus trop promotionnels ou biaisés. Un texte factuel, nuancé et sans superlatifs excessifs ("le meilleur", "révolutionnaire") sera beaucoup plus souvent repris comme source.',
    impact: '10 points',
    color: '#10A37F',
  },
  'Présence externe': {
    ...
    desc: 'Les IA croisent plusieurs sources pour évaluer votre crédibilité. Être mentionné dans la presse, sur des annuaires reconnus ou sur les réseaux sociaux renforce votre autorité perçue.',
    impact: '5 points',
    color: '#10A37F',
  },
  'Fraîcheur & maintenance': {
    ...
    desc: 'Pour les sujets qui évoluent, les IA préfèrent les contenus récents. Une date de mise à jour visible, des chiffres à jour et un copyright récent signalent que votre site est actif et fiable.',
    impact: '5 points',
    color: '#C9861A',
  },
};
  ...
  return (
    ...
    >
      <span style={{
        ...
        background: 'rgba(138,134,128,0.12)', color: '#8A8680',
        fontSize: 10, fontFamily: 'system-ui', fontWeight: 600,
        cursor: 'help', marginLeft: 6, flexShrink: 0,
      ...
      {visible && (
        <div style={{
          position: 'fixed', top: pos.top, left: Math.min(pos.left, window.innerWidth - 320),
          ...
          <div style={{
            ...
            <div style={{
              ...
              width: 12, height: 12, background: '#1A1916',
              border: '1px solid rgba(255,255,255,0.08)',
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10 }}>
            ...
            <span style={{ fontFamily: 'Georgia, serif', fontSize: 13, color: '#F7F5F2', fontWeight: 400, lineHeight: 1.3 }}>{info.title}</span>
          </div>

          <p style={{ fontSize: 12, color: 'rgba(247,245,242,0.6)', lineHeight: 1.65, fontFamily: 'system-ui', marginBottom: 12 }}>
            {info.desc}
          </p>
    ...
    <div style={{ marginBottom: 8 }}>
      <div
        ...
        style={{
          ...
          padding: '14px 20px',
          background: hovered ? group.colorLight : '#fff',
          border: `1.5px solid ${hovered ? group.color : '#E5E2DC'}`,
          borderRadius: hovered && criteria.length > 0 ? '12px 12px 0 0' : 12,
        ...
        <span style={{ marginLeft: 'auto', fontSize: 11, color: group.color, fontFamily: 'system-ui' }}>
          {hovered ? '▲ Masquer' : '▼ Voir l\'analyse'}
        </span>
      </div>
      {hovered && (
        <div
          onMouseEnter={() => setHovered(true)}
          onMouseLeave={() => setHovered(false)}
          style={{ border: `1.5px solid ${group.color}`, borderTop: 'none', borderRadius: '0 0 12px 12px', overflow: 'hidden', animation: 'fadeIn 0.5s ease' }}>
          {criteria.map((c, i) => {
              ...
              <div key={i} style={{ background: '#fff', padding: '16px 20px', borderTop: i > 0 ? '1px solid #E5E2DC' : 'none', borderLeft: `3px solid ${levelColor}` }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 10, alignItems: 'center' }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                    <span style={{ fontFamily: 'monospace', fontSize: 9, padding: '2px 8px', borderRadius: 4, background: levelColor + '18', color: levelColor }}>
...
function RecoCard({ r, index, isPaid }) {
      ...
      <div style={{ marginBottom: 10 }}>
        <div style={{ background: '#fff', border: '1px solid #E5E2DC', borderRadius: 14, overflow: 'hidden', borderLeft: `4px solid ${tag.color}`, opacity: 0.65 }}>
          <div style={{ padding: '16px 24px', display: 'flex', alignItems: 'center', gap: 10 }}>
            ...
            {r.criterion && (
              <span style={{ fontSize: 11, color: '#8A8680', fontFamily: 'monospace' }}>{r.criterion}</span>
            )}
  ...
  if (!isUnlocked && index === 0) {
    const preview = r.text.slice(0, 350);
    return (
      <div style={{ marginBottom: 10 }}>
        <div style={{ background: '#fff', border: `1px solid ${tag.border}`, borderRadius: 14, overflow: 'hidden', borderLeft: `4px solid ${tag.color}` }}>
          ...
          <div style={{ margin: '12px 24px 20px', padding: '14px 18px', background: '#F7F5F2', borderRadius: 10, display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 16 }}>
            ...
            <a href="/pricing" style={{ background: tag.color, color: '#fff', padding: '10px 20px', borderRadius: 8, fontWeight: 600, fontSize: 12, textDecoration: 'none', whiteSpace: 'nowrap', fontFamily: 'system-ui', flexShrink: 0 }}>
              Débloquer →
            </a>
          </div>
        </div>
  ...
  return (
    <div style={{ marginBottom: 10 }}>
      <div style={{ background: '#fff', border: `1px solid ${tag.border}`, borderRadius: 14, overflow: 'hidden', borderLeft: `4px solid ${tag.color}` }}>
        ...
        <div style={{ padding: '16px 24px', fontSize: 13, color: '#3A3835', lineHeight: 1.8, fontFamily: 'system-ui' }}>{r.text}</div>
      </div>
    </div>
  );
}
...
export default function Results() {
  ...
  const groups = [
    {
      id: 'lisibility',
      label: 'Lisibilité IA',
      desc: 'Votre site peut-il être lu et compris par les IA ?',
      color: '#4285F4',
      colorLight: 'rgba(66,133,244,0.08)',
      criteria: ['Extractibilité & réponse directe', 'Crawlabilité IA', 'Données structurées'],
    },
    {
      id: 'credibility',
      label: 'Crédibilité',
      desc: 'Les IA peuvent-elles vous faire confiance ?',
      color: '#10A37F',
      colorLight: 'rgba(16,163,127,0.08)',
      criteria: ['Vérifiabilité & preuves', 'Autorité & E-E-A-T', 'Neutralité éditoriale', 'Présence externe'],
    },
    {
      id: 'freshness',
      label: 'Fraîcheur',
      desc: 'Votre contenu est-il récent et maintenu ?',
      color: '#C9861A',
      colorLight: 'rgba(201,134,26,0.08)',
  ...
  function getGroupScore(group) {
    ...
    return {
      score: items.reduce((s, c) => s + c.score, 0),
      max: items.reduce((s, c) => s + c.max, 0),
    };
  }
        ...
        <div style={{ minHeight: 'calc(100vh - 56px)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '40px 24px' }}>
          <div style={{ width: '100%', maxWidth: 560 }}>
            <div style={{ textAlign: 'center', marginBottom: 48 }}>
              ...
              <h1 style={{ fontSize: 32, color: '#1A1916', letterSpacing: -1, marginBottom: 8, lineHeight: 1.1 }}>
                Audit GEO en cours{loadingDots}
              </h1>
                ...
                return (
                  <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 16, padding: '10px 0', borderBottom: i < steps.length - 1 ? '1px solid #F7F5F2' : 'none' }}>
                    ...
                    <div style={{ flex: 1 }}>
                      ...
                      {isActive && <div style={{ fontSize: 11, color: '#8A8680', fontFamily: 'system-ui', marginTop: 2, animation: 'fadeIn 0.3s ease' }}>{s.sub}</div>}
                    </div>
            ...
            <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', marginBottom: 16 }}>
              <div>
                ...
                <div style={{ fontFamily: 'Georgia, serif', fontSize: 22, color: '#1A1916', letterSpacing: -0.5 }}>
                  {result.recommendations.length} recommandations expertes
                </div>
              </div>
              {!isPaid && (
                <div style={{ fontFamily: 'monospace', fontSize: 10, color: '#D97757', background: 'rgba(217,119,87,0.08)', padding: '4px 12px', borderRadius: 20, border: '1px solid rgba(217,119,87,0.2)' }}>
                  1 aperçu · {result.recommendations.length - 1} verrouillées
                </div>
              )}
            </div>
            ...
            {!isPaid && (
              <div style={{ marginTop: 16, background: '#1A1916', borderRadius: 16, padding: '28px 32px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 20, boxShadow: '0 8px 32px rgba(26,25,22,0.12)' }}>
                <div>
                  ...
                  <div style={{ fontSize: 13, color: 'rgba(247,245,242,0.45)', fontFamily: 'system-ui', lineHeight: 1.6 }}>
                    {result.recommendations.length - 1} recommandations expertes avec méthodes, exemples et impact attendu.
                  </div>
                </div>
                <a href="/pricing" style={{ background: '#D97757', color: '#fff', padding: '14px 32px', borderRadius: 10, fontWeight: 600, fontSize: 14, textDecoration: 'none', whiteSpace: 'nowrap', fontFamily: 'system-ui', flexShrink: 0 }}>
                  Débloquer — 9€ →
                </a>
              </div>
            )}
