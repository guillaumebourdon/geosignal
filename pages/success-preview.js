/**
 * TEMPORARY preview page — renders /success in both variants without Stripe.
 * Access: /success-preview?plan=rapport or /success-preview?plan=pro
...
export default function SuccessPreview() {
  ...
  const plan = router.query.plan || 'rapport';
  const isPro = plan === 'pro';
  const email = 'guillaume@beeleven.fr';
  ...
  if (!ready) return (
    <div style={{ background: '#F7F5F2', minHeight: '100vh', fontFamily: 'Georgia, serif' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', minHeight: '60vh', padding: 24 }}>
        <div className="success-spinner" style={{ width: 44, height: 44, borderRadius: '50%', border: '3px solid #EDE8E0', borderTopColor: '#D97757', animation: 'spin 0.8s linear infinite', marginBottom: 24 }} />
  ...
  return (
    <>
      <Head>
        <title>Preview /success ({plan}) | Detekia</title>
      ...
      <div style={{ background: '#F7F5F2', minHeight: '100vh', fontFamily: 'Georgia, serif' }}>
        ...
        {/* Banner */}
        <div style={{ background: '#D97757', padding: '8px 24px', textAlign: 'center', fontFamily: 'monospace', fontSize: 11, color: '#fff', letterSpacing: 1 }}>
          PREVIEW MODE — Plan: {plan.toUpperCase()} — <a href={`/success-preview?plan=${isPro ? 'rapport' : 'pro'}`} style={{ color: '#fff', textDecoration: 'underline' }}>Basculer vers {isPro ? 'one-page' : 'Pro'}</a>
        </div>
        ...
        <div style={{ maxWidth: 720, margin: '0 auto', padding: '36px 24px 0' }}>
          <div className="reveal reveal-d3 success-cta-grid" style={{ display: 'grid', gridTemplateColumns: isPro ? '1fr' : '1fr 1fr', gap: 14, marginBottom: 40, maxWidth: isPro ? 440 : undefined, margin: isPro ? '0 auto' : undefined }}>
            <a href="mailto:hello@detekia.fr?subject=Accompagnement GEO" className="card-interactive" style={{
        ...
        <style>{`
          ...
          * { box-sizing: border-box; margin: 0; padding: 0; }
          ...
          .success-icon-pop { opacity: 0; animation: scaleIn 0.4s cubic-bezier(0.22,1,0.36,1) 0.35s forwards; }
          .success-mail-icon { animation: floatY 3s ease-in-out infinite; }
          ...
          @media (prefers-reduced-motion: reduce) {
            .success-spinner, .success-icon-pop, .success-mail-icon { animation: none !important; opacity: 1 !important; }
          }
        `}</style>
      </div>
    </>
