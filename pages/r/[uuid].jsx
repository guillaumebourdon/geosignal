...
const CASES_FR = {
  extractibilite: "SEO Vendor a obtenu 549 sessions ChatGPT en 7 mois grâce à des tactiques de citabilité (sections citables, format question-réponse directe). (SEO Vendor, 2026)",
...
const REPORT_STRINGS = {
  fr: {
    ...
    methodologyScoreCalc: "Le score global est calculé à partir des 6 critères techniques (sur 90 points) auxquels s'ajoute un bonus de neutralité éditoriale allant de -3 à +3 points, évalué par IA. Le score affiché n'est donc pas la somme directe des 7 sous-scores.",
  ...
  en: {
    ...
    methodologyP1: 'This report is generated by automated analysis of your page\'s DOM via a specialized AI scraping service and evaluation on <strong>7 weighted criteria</strong>. The Editorial Neutrality criterion is evaluated by artificial intelligence. The other 6 criteria are evaluated by technical HTML analysis.',
    methodologyP1Pro: 'Analysis performed on {n} pages prioritized by editorial importance (prioritization by sitemap + freshness + depth). Each page is evaluated on 7 weighted criteria. The Pro AI test covers 30 simulated queries (vs 10 for the one-page report).',
    ...
    methodologyScoreCalc: 'The overall score is calculated from 6 technical criteria (out of 90 points) plus an editorial neutrality bonus ranging from -3 to +3 points, evaluated by AI. The displayed score is therefore not a direct sum of the 7 sub-scores.',
    ...
    <>
      ...
      <div style={{ background: '#F7F5F2', minHeight: '100vh', fontFamily: 'system-ui,-apple-system,BlinkMacSystemFont,sans-serif' }}>
        ...
        <main role="main" aria-label={locale === 'en' ? 'GEO Report' : 'Rapport GEO'} style={{ maxWidth: 900, margin: '0 auto', padding: '32px 20px 80px' }}>
          ...
          {/* ═══ SECTION 4: 7 CRITÈRES DÉTAILLÉS ═══ */}
              ...
              return (
                <div key={i} id={`critere-${i + 1}`} style={{ background: '#fff', border: '1px solid #E5E2DC', borderRadius: 14, padding: 24, marginBottom: 20, scrollMarginTop: 70 }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 10 }}>
                    <span style={{ fontFamily: 'monospace', fontSize: 9, color: '#6B6762', letterSpacing: 2, textTransform: 'uppercase' }}>{group} · {rs(locale).criterion_s} {i + 1} / 7</span>
                  </div>
            ...
            {CRITERIA_NAMES.map((criterionName, idx) => {
              ...
              return (
                <div key={idx} id={`critere-${idx + 1}`} style={{ background: '#fff', border: '1px solid #E5E2DC', borderRadius: 14, padding: 24, marginBottom: 20, scrollMarginTop: 70 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 10 }}>
                    <span style={{ fontFamily: 'monospace', fontSize: 9, color: '#6B6762', letterSpacing: 2, textTransform: 'uppercase' }}>{rs(locale).criterion_s} {idx + 1} / 7</span>
                  </div>
                  <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', gap: 16, marginBottom: 12, flexWrap: 'wrap' }}>
